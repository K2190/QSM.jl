...
function ismv(f, mask, vsz, r, tol, maxit, verbose)
...
#   See also CG, SMVKERNEL, SHARP, VSHARP, RESHARP, PDF, LBV
    ...
    # Using this function, make sure to send 
        # verbose = false 
        # maxit = 500
        # r = 3
        # tol = 0.0
    ...
    if tol == 0.0  # tol not used
        tol = sqrt(eps(class(f)))
    end
    
    # output
    # <TODO> How to use "like" in Julia
    ...
    pad = Math.ceil(r / Math.min(vsz)) + [0, 0, 0]
...
function ismv_(f, h, bc, m, tol, maxit, verbose)
