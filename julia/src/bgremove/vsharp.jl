function vsharp(f, mask, vsz, rs, thr)
    ...
    for ii = 1:2:length(rs)
        ...
        if ii+1 <= length(rs)
            ...
            if ii == 1
                ...
                ih(abs(real(h)) < thr) = 0
                if ndims(f) > 3
                     ih = repeat(ih, outer=[1, 1, 1, size(f, 4)])
                end
            end

            # 4d support
            if ndims(f) > 3
                h = repeat(h, outer=[1, 1, 1, size(f, 4)])
                mask1n = repeat(mask1, outer=[1, 1, 1, size(f, 4)])
                mask2n = repeat(mask2, outer=[1, 1, 1, size(f, 4)])
            else
                mask1n = mask1
                mask2n = mask2
            end
        ...
        # do single kernel
            ...
            if ndims(f) > 3
                h = repeat(h, outer=[1, 1, 1, size(f, 4)])
                maskn = repeat(mask1, outer=[1, 1, 1, size(f, 4)])
            else
                maskn = mask1
            end
