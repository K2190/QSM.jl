...
function [new_img, new_M] = affine(old_img, old_M, new_elem_size, verbose, bg, method)
   ...
   elseif ~exist("bresenham_line3d.m","file") & method == 3
      error([char(10) char(10) "Please download 3D Bresenham''s line generation program 
      from:" char(10) char(10) "http://www.mathworks.com/matlabcentral/fileexchange/loadFile.do?objectId=21057" 
      char(10) char(10) "to test Fischer""s Bresenham interpolation method." char(10) char(10)]);
   end

   #  Make compatible to MATLAB earlier than version 7 (R14), which
   #  can only perform arithmetic on double data type
   #
   old_img = convert_to_double(old_img)
   ...
   for z = 1:new_dim(3)
      ...
      #
      old_M1 = old_M  
      old_M1[:,4] = old_M[:,4] - sum(old_M[:,1:3],2)
      new_M1 = new_M
      new_M1[:,4] = new_M[:,4] - sum(new_M[:,1:3],2)
   ...
   for y = 1:ydim1
      ...
      for x = 1:xdim1
         ...
         if (	old_X > 1-TINY & old_X < xdim2+TINY & 
            ...
            v000 = convert_to_double(img[x000, y000, z000])
            v010 = convert_to_double(img[x010, y010, z010])
            v001 = convert_to_double(img[x001, y001, z001])
            v011 = convert_to_double(img[x011, y011, z011])

            v100 = convert_to_double(img[x100, y100, z100])
            v110 = convert_to_double(img[x110, y110, z110])
            v101 = convert_to_double(img[x101, y101, z101])
            v111 = convert_to_double(img[x111, y111, z111])
