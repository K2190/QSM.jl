...
function [nii, orient, pattern] = rri_orient(nii, varargin)
   ...
   dim = convert_to_double(nii.hdr.dime.dim([2:4]))
   ...
   for i = 1:3
      if flip_orient(i) & ~isequal(convert_to_double(tmp(i)), 0)
         tmp[i] = int16(convert_to_double(new_dim[i]) - convert_to_double(tmp[i]) + 1)
      end
   end
