...
function write_ext(ext, io)

   write(io, ext.extension) #, "uchar")

   for i=1:ext.num_ext
      fwrite(io, ext.section[i].esize)     #, "int32")
      fwrite(io, ext.section[i].ecode)     #, "int32")
      fwrite(io, ext.section[i].edata)     #, "uchar")
   end
