...
function verify_nii_ext(ext)
   ...
   #    ext.section(i).edata = [ext.section(i).edata zeros(1,ext.section(i).esize-length(ext.section[i].edata)-8)]
   #    esize_total = esize_total + ext.section(i).esize
   # end

   return [ext, esize_total]                                      # verify_nii_ext
end
