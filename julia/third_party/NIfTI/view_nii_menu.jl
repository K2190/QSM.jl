...
function reset_zoom(fig)
   ...
   setappdata(get(gca,"zlabel"), "ZOOMAxesData", [get(gca, "xlim") get(gca, "ylim")])
#   zoom reset
 #  zoom getlimits
   zoom out
...
function img_info
   ...
   max_value = num2str(convert_to_double(max(nii_view.nii.img[:])))
   min_value = num2str(convert_to_double(min(nii_view.nii.img[:])))

   dim = sprintf("%d  %d  %d", convert_to_double(hdr.dime.dim(2:4)))
   vox = sprintf("%.3f  %.3f  %.3f", convert_to_double(hdr.dime.pixdim(2:4)))

   if convert_to_double(hdr.dime.datatype) == 1
      type = "1-bit binary"
   elseif convert_to_double(hdr.dime.datatype) == 2
      type = "8-bit unsigned integer"
   elseif convert_to_double(hdr.dime.datatype) == 4
      type = "16-bit signed integer"
   elseif convert_to_double(hdr.dime.datatype) == 8
      type = "32-bit signed integer"
   elseif convert_to_double(hdr.dime.datatype) == 16
      type = "32-bit single float"
   elseif convert_to_double(hdr.dime.datatype) == 64
      type = "64-bit convert_to_double precision"
   elseif convert_to_double(hdr.dime.datatype) == 128
      type = "24-bit RGB true color"
   elseif convert_to_double(hdr.dime.datatype) == 256
      type = "8-bit signed integer"
   elseif convert_to_double(hdr.dime.datatype) == 511
      type = "96-bit RGB true color"
   elseif convert_to_double(hdr.dime.datatype) == 512
      type = "16-bit unsigned integer"
   elseif convert_to_double(hdr.dime.datatype) == 768
      type = "32-bit unsigned integer"
   elseif convert_to_double(hdr.dime.datatype) == 1024
      type = "64-bit signed integer"
   elseif convert_to_double(hdr.dime.datatype) == 1280
...
function editvox
   ...
   if nii_view.nii.hdr.dime.datatype == 128
      imgvalue = [convert_to_double(nii.img(sag,cor,axi,1,nii_view.scanid)) convert_to_double(nii.img(sag,cor,axi,2,nii_view.scanid)) convert_to_double(nii.img(sag,cor,axi,3,nii_view.scanid))]
      init_val = sprintf("%7.4g %7.4g %7.4g",imgvalue)
   elseif nii_view.nii.hdr.dime.datatype == 511
      R = convert_to_double(nii.img(sag,cor,axi,1,nii_view.scanid)) * (nii_view.nii.hdr.dime.glmax - 
		nii_view.nii.hdr.dime.glmin) + nii_view.nii.hdr.dime.glmin
      G = convert_to_double(nii.img(sag,cor,axi,2,nii_view.scanid)) * (nii_view.nii.hdr.dime.glmax - 
		nii_view.nii.hdr.dime.glmin) + nii_view.nii.hdr.dime.glmin
      B = convert_to_double(nii.img(sag,cor,axi,3,nii_view.scanid)) * (nii_view.nii.hdr.dime.glmax - 
		nii_view.nii.hdr.dime.glmin) + nii_view.nii.hdr.dime.glmin
      imgvalue = [R G B]
      init_val = sprintf("%7.4g %7.4g %7.4g",imgvalue)
   else
      imgvalue = convert_to_double(nii.img(sag,cor,axi,nii_view.scanid))
   ...
   while repeat
      ...
      if ( (nii_view.nii.hdr.dime.datatype == 128 | nii_view.nii.hdr.dime.datatype == 511) && length(imgvalue) != 3 ) | 
         ( (nii_view.nii.hdr.dime.datatype != 128 & nii_view.nii.hdr.dime.datatype != 511) && length(imgvalue) != 1 )
	       println("Not here...\n")# do nothing
      else
         repeat = 0
      end
...
function img_hist
   ...
   nii_view = getappdata(gcbf, "nii_view")
   N = hist(convert_to_double(nii_view.nii.img[:]),256)
   x = linspace(convert_to_double(min(nii_view.nii.img[:])), convert_to_double(max(nii_view.nii.img[:])), 256)
   figurebar(x,N)
