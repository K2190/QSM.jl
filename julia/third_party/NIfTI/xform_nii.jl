...
function xform_nii(nii, tolerance, preferredForm)
   ...
   #
   is_member_arr = in.(nii.hdr.dime.datatype, [2,4,8,16,64,256,512,768])
   is_member = length(findall( x -> x == 1, is_member_arr ))

   if nii.hdr.dime.scl_slope != 0 && is_member == 1 && (nii.hdr.dime.scl_slope != 1 || nii.hdr.dime.scl_inter != 0)
   ...
   #
   if nii.hdr.dime.scl_slope != 0 && (nii.hdr.dime.datatype == 32 || nii.hdr.dime.datatype == 1792)

      nii.img = nii.hdr.dime.scl_slope * convert_to_double(nii.img) + nii.hdr.dime.scl_inter;

      if nii.hdr.dime.datatype == 32
         nii.img = convert_to_single(nii.img)
      end

      nii.hdr.dime.glmax = max(convert_to_double(nii.img[:]))
      nii.hdr.dime.glmin = min(convert_to_double(nii.img[:]))
   ...
   #
   filename = join([nii.fileprefix ".mat"])
   if nii.filetype == 0 && isfile(filename)
      load(filename)	      # old SPM affine matrix
      R=M[1:3,1:3]
      T=M[1:3,4]
      T=R*ones(3,1)+T
      M[1:3,4]=T
   ...
   elseif nii.filetype == 0
      #<TODO> Check if this is used. This is not a part of original data_history struct
   ...
   if orient[1] != 1 || orient[2] != 2 || orient[3] != 3
      ...
      for i = 1:3
         tmpVal = flip_orient[i]
         if tmpVal > 0
            if ~isempty(pattern)
               pattern = reverse(pattern, dims=i)
            else
               nii.img = reverse(nii.img, dims=i)
            end
         end
      end
      ...
      #
      tmp = get_dhist_originator()  #hdr.hist.originator[1:3]
      tmp = tmp[rot_orient]
      flip_orient = flip_orient[rot_orient]

      for i = 1:3
         if flip_orient[i] > 0 && ~isequal(tmp[i], 0)
      ...
      set_dhist_originator(tmp)
      set_dhist_flip_orient(rot_orient)
      set_dhist_rot_orient(flip_orient)
      ...
      if ~isempty(pattern)
         ...
         if hdr.dime.datatype == 32 || hdr.dime.datatype == 1792 ||
            ...
            nii.img[:,:,:,1] = permutedims(nii.img[:,:,:,1], rot_orient)
            nii.img[:,:,:,2] = permutedims(nii.img[:,:,:,2], rot_orient)

            if hdr.dime.datatype == 128 | hdr.dime.datatype == 511
               nii.img[:,:,:,3] = permutedims(nii.img[:,:,:,3], rot_orient)
            end
         else
            nii.img = permutedims(nii.img, rot_orient)
         end
      end
   else
      # hdr.hist.rot_orient = []
      # hdr.hist.flip_orient = []
      set_dhist_flip_orient([0, 0, 0])
      set_dhist_rot_orient([0,0,0])
   end

   nii.hdr = hdr

   return nii					# xform_nii
end
...
function  change_hdr(hdr, tolerance, preferredForm)
   ...
   if useForm == 's'
      R = cat([hdr.hist.srow_x[1] hdr.hist.srow_x[2] hdr.hist.srow_x[3];
               hdr.hist.srow_y[1] hdr.hist.srow_y[2] hdr.hist.srow_y[3];
               hdr.hist.srow_z[1] hdr.hist.srow_z[2] hdr.hist.srow_z[3]], dims=2)

      T = [hdr.hist.srow_x[4],
           hdr.hist.srow_y[4],
           hdr.hist.srow_z[4]]
         
      nonZeroR = jl_findNonZeroArray(R)
      sumR = sum(R, dims=1)

      dummyValToSkip = false;
      if dummyValToSkip && (det(R) == 0 || R[find(R)] != sum(R)')
         ...
         if det(R) == 0 | ~isequal(R(find(R)), sum(R)')
            msg = "   Non-orthogonal rotation or shearing "
            msg = msg * "found inside the affine matrix" 
            msg = msg * "   in this NIfTI file. You have 3 options:"
            msg = msg * "   1. Using included reslice_nii.m program to reslice the NIfTI"
            msg = msg * "      file. I strongly recommand this, because it will not cause"
            msg = msg * "      negative effect, as long as you remember not to do slice"
            msg = msg * "      time correction after using reslice_nii.m."
            msg = msg * "   2. Using included load_untouch_nii.m program to load image"
            msg = msg * "      without applying any affine geometric transformation or"
            msg = msg * "      voxel intensity scaling. This is only for people who want"
            msg = msg * "      to do some image processing regardless of image orientation"
            msg = msg * "      and to save data back with the same NIfTI header."
            msg = msg * "   3. Increasing the tolerance to allow more distortion in loaded"
            msg = msg * "      image, but I don't suggest this."
            msg = msg * "   To get help, please type:  help reslice_nii.m"
            msg = msg * "   help load_untouch_nii.m  help load_nii.m"
            println(msg)
         end
      end
   ...
   if affine_transform == 1
      voxel_size = abs.(sum(R, dims=1))
      inv_R = inv(R)

      # Repalce -0.0 with 0.0
      Rindx = findall( x -> x == -0.0,  inv_R)
      for i=1:length(Rindx)
	      inv_R[Rindx[i]] = 0.0
      end

      # THIS IS A BUG IN JULIA
      # https://github.com/JuliaLang/julia/issues/15305
      # originator = zeros(Float32, (1, 3))
      originator = inv_R.*(-T) 
      originator = sum(originator, dims=1)
      originator .+= 1
      ...
      set_dhist_originator(originator)
...
function get_units(hdr)
   ...
   tmpVal = hdr.dime.xyzt_units & 7	# mask with 0x07
   if tmpVal == 1
   ...
   tmpVal = hdr.dime.xyzt_units & 56	# mask with 0x38
   if tmpVal == 16
...
function jl_findNonZeroArray(inp_array)
	cart_indicies = findall( x -> x != 0.0,  inp_array)
   num_indicies = length(cart_indicies)
	non_zero_array = zeros(num_indicies)
	for i = 1:length(cart_indicies)
	   value = inp_array[cart_indicies[i]]
	   non_zero_array[i] = value
	end
