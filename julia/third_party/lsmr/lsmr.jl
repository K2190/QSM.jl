...
#Not needed, as this used directly in calling location
...
function lsmr(A, b, lambda, atol, btol, conlim, itnlim, localSize, show)
...
#-----------------------------------------------------------------------
    ...
    if localSize > 0
      ...
      localV = zeros(n, minimum([localSize minDim]))
    end
    ...
    # Save normA here to have global scope
    normA_gl = 0.0
    condA_gl = 0.0
    normx_gl = 0.0
  ...
  #     Main iteration loop.
    while itn < itnlim
      ...
      if istop > 0
        # Update values to send back in return below
        normA_gl = normA
        condA_gl = condA
        normx_gl = normx
        break
    ...
    return  x #[x, istop, itn, normr, normAr, normA_gl, condA_gl, normx_gl]
